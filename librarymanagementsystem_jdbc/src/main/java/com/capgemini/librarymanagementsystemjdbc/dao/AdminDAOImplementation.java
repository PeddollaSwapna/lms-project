...
public class AdminDAOImplementation implements AdminDAO{
	...
	public boolean issueBook(int bId,int uId) {
		conn=Utility.getConnection();
		try(PreparedStatement pst = conn.prepareStatement(QueryMapping.issueBookQuery);){
			pst.setInt(1, bId);
			rs = pst.executeQuery();
			if(rs.next()) {
				try(PreparedStatement pstmt = conn.prepareStatement(QueryMapping.issueBookQuery);) {
					pstmt.setInt(1, uId);
					pstmt.setInt(2, bId);
					pstmt.setInt(3, uId);
					rs = pstmt.executeQuery();
					if(rs.next()) {
						try(PreparedStatement pstmt1 = conn.prepareStatement(QueryMapping.issueBookQuery);){
							pstmt1.setInt(1, bId);
							pstmt1.setInt(2, uId);
							...
							if(count != 0) {	
								try(PreparedStatement pstmt2 = conn.prepareStatement(QueryMapping.issueBookQuery);){
									pstmt2.setInt(1, uId);
									pstmt2.setInt(2, bId);
									pstmt2.setInt(3, uId);
